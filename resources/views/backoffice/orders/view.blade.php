...
@section('title', config('corvus.app_name') . ' | ' . __('labels.orders.view'))
...
<div class="card mt-2">
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-12">
            @if (Auth::user()->hasRoles(['administrator', 'orders_staff']))
            @if(in_array($order->status, $allowed_status))
                <a class="btn btn-primary m-2 float-right" href="{{ route('backoffice.orders.update', $order->id) }}">
                    <i class="fas fa-sync-alt align-middle"></i> <span class="align-middle">
                        Process Order</span></a>
            @endif

            @if($order->status_slug === 'APPROVED')
                <a class="btn btn-info m-2 float-right" href="{{ route('backoffice.orders.update', $order->id) }}">
                    <i class="fas fa-sync-alt align-middle"></i> <span class="align-middle">
